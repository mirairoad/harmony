...
function Block({ block }: { block: BlockType }) {
  switch (block.type) {
    case "p":
      return (
        <p class="text-base sm:text-base text-base-content/80 leading-relaxed my-3">
          {block.text}
        </p>
      );
    case "code":
...
export default function ProjectPage(ctx: Context<State>): JSX.Element {
  ...
  return (
    <>
      <Head>
        <title>{project.title} — {ctx.state.client?.title ?? "CV"}</title>
        <meta name="description" content={project.tagline} />
      </Head>

      <div class="relative min-h-screen bg-base-100 bg-dot-grid bg-size-[28px_28px] pt-24 sm:pt-32 pb-32 sm:pb-20">
        <article class="max-w-3xl mx-auto px-5 sm:px-8">
          <a
            href="/projects"
            class="inline-flex items-center gap-1 text-sm text-base-content/50 hover:text-primary mb-8 transition-colors"
          >
            ← All projects
          </a>

          <header class="mb-10 pb-7 border-b border-base-300">
            ...
            <p class="text-lg text-base-content/70 leading-relaxed mb-5">
              {project.tagline}
            </p>
            ...
            <div class="flex flex-wrap gap-1.5 mb-5">
              {project.stack.map((s) => (
                <span
                  key={s}
                  class="badge badge-sm bg-base-200 border-base-300 font-mono text-[11px]"
                >
                  {s}
                </span>
              ))}
              ...
              <div class="flex flex-wrap gap-2">
                {project.links.map((l) => (
                  <a
                    ...
                    class="btn btn-sm rounded-lg btn-ghost border border-base-300 hover:border-primary/50 hover:bg-primary/10 font-mono text-xs"
                  >
                    {l.label} ↗
                  </a>
                ))}
          ...
          {project.sections.length > 2 && (
            <div class="bg-base-300/60 rounded-2xl border border-base-300 p-4 mb-8 shadow-sm">
          ...
          {project.sections.map((section) => (
            <section
              key={section.id}
              id={section.id}
              class="mb-10 scroll-mt-24 sm:scroll-mt-8"
            >
          ...
          <div class="flex justify-between gap-4 mt-12 pt-6 border-t border-base-300">
            {prev
              ? (
                <a href={`/projects/${prev.slug}`} class="group flex flex-col max-w-xs py-2">
