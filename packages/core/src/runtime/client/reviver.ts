...
export function maybeHideMarker(marker: Comment): Comment | Text {
  ...
  text._harmonyMarker = marker.data;
...
export function domToVNode(
  ...
  while (sib !== null) {
    ...
    // deno-lint-ignore no-explicit-any
    if ((sib as any)._harmonyRootFrag || sib instanceof DocumentFragment) {
      ...
      const comment = isFreshMarkerText(sib) ? sib._harmonyMarker : sib.data;
      if (comment.startsWith("frsh:")) {
        const parts = comment.split(":");
...
export function isElementNode(node: Node): node is HTMLElement {
  return node.nodeType === Node.ELEMENT_NODE && !("_harmonyRootFrag" in node);
}
...
): HTMLElement & { _harmonyRootFrag: boolean } {
  // @ts-ignore this is fine
  const rootFrag = {
    _harmonyRootFrag: true,
    nodeType: 1,
    parentNode: parent,
    nextSibling: null,
    get firstChild() {
