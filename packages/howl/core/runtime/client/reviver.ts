...
export function domToVNode(
  ...
  while (sib !== null) {
    ...
    // deno-lint-ignore no-explicit-any
    if ((sib as any)._howlRootFrag || sib instanceof DocumentFragment) {
      ...
      for (let i = 0; i < sib.attributes.length; i++) {
        ...
        props[attr.nodeName] =
          // deno-lint-ignore no-explicit-any
          typeof (sib as any)[attr.nodeName] === "boolean" ? true : attr.nodeValue;
      }
...
export function isElementNode(node: Node): node is HTMLElement {
  return node.nodeType === Node.ELEMENT_NODE && !("_howlRootFrag" in node);
}
...
): HTMLElement & { _howlRootFrag: boolean } {
